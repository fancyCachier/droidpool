...
操作人员可以在设备墙（`https://droidpool.daboshi.cn`，内网可达）上接管任意一台设备，比如帮你扫码、
处理需要真人判断的弹窗。
...
- **点一格放大**：H.264 实时画面（走 `https://droidpool.daboshi.cn`；`http://192.168.14.32:8600` 上的页面会自动跳过去，原因见方案对比文档 §5.6.2），指针实时转发：点击 = 点按，按住 = 长按，拖动 = 跟手拖动/滑动，Alt(Option)/Ctrl + 拖动 = 双指缩放，滚轮 = 滚动。
  点一下画面后可直接用物理键盘打字（含 Ctrl/⌘ 组合键、Esc = 返回）；中文用侧栏输入框，经设备剪贴板粘贴。
  ⌘/Ctrl+C 取设备剪贴板、⌘/Ctrl+V 把本机剪贴板粘进设备。同时最多 4 台设备开实时通道。
...
| 操作没反应 | 看设备墙是否有人接管中；页面顶部状态栏会写明当前是实时通道还是截图流 |
| 放大后一片黑 | 先看状态栏与 HUD：「静止 N s」说明画面本来就没变（scrcpy 只在变化时出帧），不是坏了；真出问题时页面会弹出原因并自动退回截图流 |
