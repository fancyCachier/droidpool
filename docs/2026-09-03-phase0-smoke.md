...
### 3.2 dpi 已按收银机实机定：1366×768 @ 160

首次发烟用的 2560×1600 @ 320 dpi 只有 1280×800 dp，登录页竖向卡片放不下，「登 录」按钮在首屏之下要滑一次。当天下午用 adb 连上收银机实机读数（配对 `192.168.180.200`，设备 `D1s_d_2nd-M`，SUNMI，Android 11，`armeabi-v7a`）：`wm size` = **1366x768**，`wm density` = **160**，`ro.sf.lcd_density` = 160。**默认 profile 据此定为 1366×768 @ 160 dpi**，`bench/redroid-up.sh` 与路线图 `boot_args` 已同步。副作用是截图从 3 MB 降到不足 1 MB，screencap 更快。

附带事实：实机是 32 位 `armeabi-v7a`，池里跑的是 `_64only` 镜像上的 arm64 构建；UI 验证无差别，涉及 native 插件的 32 位问题池里测不出，记入不可测项。

重建容器后核对：redroid `wm size` 1366x768、`wm density` 160，boot 11 s，app 冷启 3.2 s，登录页截图 88 KB，screencap 稳态 **0.3 s**（原 0.77 s）。系统栏差异：redroid 应用区 1366×708（AOSP 状态栏 + 三键导航栏），Sunmi 应用区 1366×720，相差 12 px，布局其余一致。Sunmi 另有一块 1024×600 @ 213 dpi 的第二显示（客显），池里不模拟。
