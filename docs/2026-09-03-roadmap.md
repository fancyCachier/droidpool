...
| GPU | Mali-G610 归 Arm 专有 `mali` 驱动，只有 `/dev/mali0`；`/dev/dri/renderD128` 是显示子系统、`renderD129` 是 NPU，都不是 GPU | ~~GPU host 模式有尝试基础~~ **本行原记录有误，2026-09-06 更正**，见 `2026-09-06-gpu-host-spike.md` |
...
| GPU host 模式作为 Day-1 依赖 | 不建议 | 原因是 Mesa 兼容性未知；2026-09-06 spike 已证伪整条路，见 `2026-09-06-gpu-host-spike.md` |
...
8. ~~**专项 B：GPU host（限时半天）**~~ **2026-09-06 已做，不通**。`gralloc.gbm` 建不出 gbm device，SurfaceFlinger 起不来，容器无限重启。内核、宿主、镜像三层各自缺条件，不是配置问题。详见 `2026-09-06-gpu-host-spike.md`（附带测出瓶颈在渲染而非编码）。
...
- ~~GPU host 模式（专项 B 的续篇）~~ 2026-09-06 spike 判定不做，见 `2026-09-06-gpu-host-spike.md` §4。
