...
1. 在 .54 起一个容器：
   ...
   docker run -d --privileged --name redroid-smoke \
     ...
     androidboot.redroid_width=2560 androidboot.redroid_height=1600 \
     androidboot.redroid_dpi=320 androidboot.redroid_gpu_mode=guest
   ```
   `use_memfd=true` 不可省。分辨率与现有 AVD `cashier_tablet` 一致，agent 已有坐标经验可复用。第一次启动同时回答 binderfs 自举问题。
...
boot_args   = "androidboot.use_memfd=true androidboot.use_redroid_overlayfs=1 androidboot.redroid_width=2560 androidboot.redroid_height=1600 androidboot.redroid_dpi=320 androidboot.redroid_gpu_mode=guest"
```
